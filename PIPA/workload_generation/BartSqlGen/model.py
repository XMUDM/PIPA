...
class BartSqlGenModel(BartPretrainedModel):
    ...
    def train(self):
        ...
        total_epoch_loss = []
        # self.train_data, self.test_data, self.BartSqlGen, optimizer = accelerator.prepare(self.train_data, self.test_data, self.BartSqlGen, optimizer)
        ...
        for epoch in range(self.num_epochs):
            ...
            if epoch < 15:
                for batch in self.train_data:
                    ...
                    # logging.info("labels:" + str(labels[0]))
                    # logging.info("input:" + str(input_ids[0]))
                ...
                for batch in self.train_data:
                    ...
                    for i in range(label_count - input_count):
                        ...
                        # print(values)
                        # print(predictions)
                        # print(self.tok.decode(labels[0,masked_index]))
    ...
    def evaluate(self):
        ...
        # self.save_txt(self.index_label_list,"./result/index_label_list.txt")
        # self.save_txt(self.index_generate_list,"./result/index_generate_list.txt")
        ...
        # index_metrics = compute_metrics("./result/index_label_list.txt", "./result/index_generate_list.txt")
        # logging.warning("index_metrics: " + str(index_metrics))
    ...
    def sqlmask(self, labels, sqlmask):
        try:
            ...
            while done == 0:
                run_times += 1
                # logging.warning("run_times: " + str(run_times))
                ...
                if flag_done:
                    ...
                    # logging.warning("flag done!")
                    # logging.warning("1: " + str(new_token))
                    ...
                    for ix in range(len(action)):
                        if not (self.number_start <= action[ix][0] <= self.number_end):
                            flag_number = False
                    if flag_number:
                        # logging.warning(len(action))
                        rd_num = random.randint(0, len(action) - 1)
                        # logging.warning(rd_num)
                        new_tokens = []
                    ...
                    for x in range(len(candidate_action)):
                        if flag_select or flag_order_by:
                            if self.attr_start <= x <= self.attr_end and candidate_action[x] == 1:
                                flag_attr = True
                        # if candidate_action[x] == 1:
                        #     logging.info("availaction:" + str(self.env.c_obj.num_word_map_seperate[x]))
                    '''if flag_from:
                        flag_from_join = False'''
                    if flag_select:
                        flag_select_attr = False
                    ...
                    if flag_from and not flag_from_join and not flag_from_join_end and new_token == " join":
                        ...
                        flag_end = True
                    # logging.info("new_token:K = " + str(new_token) + ":1")
                    action = [
                    ...
                    if (not flag_from_join or flag_from_join_start) and not flag_select_attr and not flag_order_by_attr:
                        ...
                        while len(action) == 0 and k <= 1000:
                            ...
                            # logging.warning(str(k) + ": " +str(new_token))
                            # logging.info("new_token:K = " + str(new_token) + ":" + str(k))
                            if flag_from and not flag_from_join and not flag_from_join_end and new_token == " join":
                                flag_from_join = True
                                break
                            elif flag_select and not flag_select_attr_end and not flag_dot and flag_attr and new_token == ",":
                    ...
                    if k == 50265 or ((flag_select and not flag_select_attr_end and not flag_order_by) or (
                        ...
                        break
                    # logging.info("last choose : " + new_token)
                    # logging.info("still remain : " + str(action))
                    if new_token == " where":
                    ...
                    if flag_from and flag_from_join_start and action and self.table_begin <= action[0][
                        0] <= self.table_end:
                        # logging.warning("join part!")
                        done = self.env.step(action[0][0])
                        current_state = action[0][0]
                        # logging.info(self.env.get_sql())
                        if len(action[0][1]) == 0:
                    ...
                    if flag_query:
                        ...
                        # logging.warning("not flag_done!")
                        # logging.warning(action)
                        if new_token == " by":
                            flag_order_by = True
                            flag_from = flag_select = False
                            flag_dot = True
                        # if len(candidate_action) == 1 and len(candidate_action[0][1]) >= 2:
